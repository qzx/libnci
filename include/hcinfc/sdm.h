 ...
 * configuration, so hci_sdm_verify() takes the exact byte range the CMAC must
 ...
 * Returns HCI_OK and sets out->mac_valid=true when the CMAC matches;
 * HCI_E_AUTH (out still filled, mac_valid=false) on MAC mismatch; or another
 * negative hci_status on a decode/crypto error. */
