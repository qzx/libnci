...
#define STEP(call, what) do { \
    if ((call) != PN7160_OK) { printf("  FAIL: %s\n", what); return; } \
    printf("  ok:   %s\n", what); } while (0)
...
static void run_workflow(pn7160 *p, uint32_t aid, const uint8_t key[16], uint8_t file_no)
{
    uint8_t zero[16] = {0};

    uint8_t comm = PN7160_DESFIRE_FULL;
    printf("[application %06X]\n", aid);
    STEP(pn7160_desfire_select_application(p, aid), "SelectApplication");
    STEP(pn7160_desfire_authenticate_ev2(p, 0, key), "AuthenticateEV2First (key 0)");

    /* ---- key management ---- */
    printf("[keys: rotate key 1 and revert]\n");
    uint8_t v = 0xFF;
    STEP(pn7160_desfire_get_key_version(p, 1, &v), "GetKeyVersion(1)");
    printf("        key 1 version before = 0x%02X\n", v);
    static const uint8_t newk[16] = { 0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,
                                      0x99,0xAA,0xBB,0xCC,0xDD,0xEE,0xFF,0x00 };
    STEP(pn7160_desfire_change_key(p, 1, zero, newk, 0x42), "ChangeKey(1) -> v0x42");
    STEP(pn7160_desfire_get_key_version(p, 1, &v), "GetKeyVersion(1)");
    printf("        key 1 version after  = 0x%02X %s\n", v, v == 0x42 ? "(CHANGED!)" : "(?)");
    STEP(pn7160_desfire_change_key(p, 1, newk, zero, 0x00), "ChangeKey(1) -> revert");
    STEP(pn7160_desfire_get_key_version(p, 1, &v), "GetKeyVersion(1)");
    ...
    pn7160_desfire_delete_file(p, file_no);
    if (!pn7160_desfire_session_active(p))
        STEP(pn7160_desfire_authenticate_ev2(p, 0, key), "re-auth after cleanup");
    STEP(pn7160_desfire_create_std_data_file(p, file_no, -1, comm, 0x0000, 64),
    ...
    STEP(pn7160_desfire_write_data(p, comm, file_no, 0, payload, 64), "WriteData (enciphered, 64 B)");
    uint8_t rb[64]; size_t rn = 0;
    STEP(pn7160_desfire_read_data_comm(p, comm, file_no, 0, 64, rb, sizeof rb, &rn), "ReadData (enciphered, 64 B)");
    printf("        readback %s: \"%s\"\n",
           (rn == 64 && memcmp(rb, payload, 64) == 0) ? "MATCHES" : "mismatch", (char *)rb);
    uint8_t fs[32]; size_t fsn = 0;
    if (pn7160_desfire_get_file_settings(p, file_no, fs, sizeof fs, &fsn) == PN7160_OK)
        hexline("GetFileSettings: ", fs, fsn);
    STEP(pn7160_desfire_delete_file(p, file_no), "DeleteFile (cleanup)");

    /* ---- PICC: create + delete a temporary application ---- */
    printf("[PICC: temp application A000FE]\n");
    STEP(pn7160_desfire_select_application(p, 0x000000), "SelectApplication(PICC)");
    STEP(pn7160_desfire_authenticate_ev2(p, 0, key), "AuthenticateEV2First (PICC master)");
    pn7160_desfire_delete_application(p, 0x0000FE);   /* clear leftover (may error) */
    if (!pn7160_desfire_session_active(p))
        STEP(pn7160_desfire_authenticate_ev2(p, 0, key), "re-auth after cleanup");
    STEP(pn7160_desfire_create_application(p, 0x0000FE, 0x0F, 0x83),
         "CreateApplication(A000FE, 3 AES keys)");
    STEP(pn7160_desfire_delete_application(p, 0x0000FE), "DeleteApplication(A000FE)");
...
int main(int argc, char **argv)
{
    ...
    while (!g_stop && !did) {
        ...
        if (pn7160_tag_supports_apdu(p)) { run_workflow(p, aid, key, file_no); did = 1; }
