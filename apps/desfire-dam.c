...
#define ST() pn7160_desfire_last_status(P)
...
static int reauth(uint8_t key_no)
{
    for (int a = 0; a < 8; a++) {
        hci_tag t; int got = 0;
        for (int i = 0; i < 10 && !got; i++) if (hci_poll(P, &t, 300) == HCI_POLL_TAG) got = 1;
        if (got) {
            pn7160_desfire_select_application(P, 0x000000);
            if (pn7160_desfire_authenticate(P, key_no, ZERO) == PN7160_OK) return 1;
        }
        hci_resume_discovery(P);
    }
    return 0;
}
...
static int cmd_info(uint16_t slot)
{
    ...
    uint8_t info[8];
    int r = pn7160_desfire_dam_get_info(P, slot, info);
    if (r == PN7160_OK) {
...
static int cmd_scan(uint16_t start, uint16_t end)
{
    ...
    for (uint32_t s = start; s <= end; s++) {
        ...
        uint8_t info[8];
        if (pn7160_desfire_dam_get_info(P, (uint16_t)s, info) == PN7160_OK) {
            uint32_t aid = info[5] | (info[6] << 8) | (info[7] << 16);
...
static int cmd_create(uint32_t aid, uint16_t slot, uint16_t quota)
{
    ...
    int r = pn7160_desfire_dam_create(P, aid, slot, 0x00, quota, 0x0F, 0x82,
                                      ZERO, ZERO, ZERO, 0x00);
    if (r == PN7160_OK) {
...
static int cmd_delete(uint32_t aid)
{
    ...
    int r = pn7160_desfire_delete_application(P, aid);
    printf("delete AID=%06X: %s (st=0x%02X)\n", aid, r == PN7160_OK ? "OK" : "FAIL", ST());
    return r == PN7160_OK ? 0 : 1;
}
...
int main(int argc, char **argv)
{
    ...
    P = hci_open(NULL, NULL);
    if (!P) { fprintf(stderr, "open failed\n"); return 1; }
    if (hci_start_discovery(P, HCI_TECH_A) != HCI_OK) { hci_close(P); return 1; }

    hci_tag t; int f = 0;
    for (int i = 0; i < 20 && !f; i++) { if (hci_poll(P, &t, 400) == HCI_POLL_TAG) f = 1; }
    pn7160_desfire_version v;
    if (!f || pn7160_desfire_get_version(P, &v) != PN7160_OK) { printf("no DESFire tag\n"); hci_close(P); return 1; }
    printf("%s uid=", pn7160_desfire_product(&v));
