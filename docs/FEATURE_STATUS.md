...
| 100 | Proximity Check | 🟢 unblocked (spec recovered) | PreparePC + ProximityCheck already run live; the missing VerifyPC MAC is now known from Proxmark3: `trunc_even(AES-CMAC(VC/PC-key, 0xFD‖OPT‖pubRespTime‖interleave(RndR,RndC)))`. Earlier crack failed only on the **key** (used SesAuthMACKey; should be the VC/PC key 0x20-0x23, default all-zero). Non-destructive - safe to finish. See [PC_DAM_REVERSE_ENGINEERING.md](PC_DAM_REVERSE_ENGINEERING.md) |
| 101 | Delegated App Mgmt | 🟢 unblocked (reference found) | Portable from Proxmark3 `hf mfdes createdelegateapp`/`getdelegateappinfo`. DAM keys 0x10-0x12; needs PICC DAM quota provisioned (use the sacrificial card; record keys in cards.keys). Higher card-breaking risk than PC - see the report |
