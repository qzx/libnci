...
| 101 | Delegated App Mgmt | 🟡 CLI + algorithm built; create needs EV1 SM | **`apps/desfire-dam.c`** CLI (scan/info/create/delete/lifecycle) + **`src/desfire_dam.c`** (the appdata/cryptogram/DAMMAC builder ported clean-room from Proxmark3, CreateDelegatedApplication 0xC9 as C9+AF, GetDelegatedInfo 0x69). **GetDelegatedInfo validated** live (2-byte slot; slot → 0xA0 not-provisioned). **CreateDelegatedApplication blocked**: the DAM keys are **2TDEA on the EV1 secure channel** (Proxmark3 defaults `T_3DES`/`DACEV1`; EV2First on key 0x10 → 0x9D), so the create command MAC needs **EV1 (2TDEA) secure messaging** - a transport layer libhcinfc doesn't have yet (it has EV2/EV3-AES + LRP, and EV1/legacy *auth*, but not EV1 SM command framing). DAMMAC/cryptogram/byte-layout are correct (Proxmark3-verified). See [PC_DAM_REVERSE_ENGINEERING.md](PC_DAM_REVERSE_ENGINEERING.md) |
