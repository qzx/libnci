...
| 100 | Proximity Check | 🟡 blocked on spec | Empirically: **PreparePC works post-auth** (→ OPT=01, pubRespTime=0320, PPS=00) and **ProximityCheck returns 7 RndR for 7 RndC** - the prepare+measure steps run. **VerifyPC** is the gap: its CMAC layout/key/truncation isn't documented in the provided overviews, and the card's non-standard statuses (0x90 after PC cmds, 0x0B/0x0C from VerifyPC) aren't a clean wrong-MAC oracle, so the 8 input orderings tried (with SesAuthMACKey, even-byte trunc) were all rejected. Needs the DS4870xx VerifyPC MAC definition |
