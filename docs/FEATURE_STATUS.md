...
| 101 | Delegated App Mgmt | 🟢 portable; not validatable on this card | Format portable from Proxmark3 `hf mfdes createdelegateapp`. Probed live: GetDelegatedInfo (0x69) takes a **2-byte slot no**; slot 0 → **0xA0 APPLICATION_NOT_FOUND** = this factory card has **no DAM slots/quota provisioned**, so CreateDelegatedApplication has nothing to create. Provisioning DAM is an owner-side, irreversible-leaning operation (DAM keys 0x10-0x12 + quota, NDA ch 6.6.2) - validating DAM needs a DAM-provisioned card. Higher card-breaking risk than PC; see [PC_DAM_REVERSE_ENGINEERING.md](PC_DAM_REVERSE_ENGINEERING.md) |
