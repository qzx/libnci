...
int mfc_write(apdu_fn fn, void *ctx, uint8_t block, const uint8_t in[16])
{
    ...
    if (fn(ctx, c1, sizeof c1, rx, sizeof rx, &rn) < 0) return PN7160_ERR;
    if (!xchg_ack(rx, rn)) { LOGE("mfc: write block %u command not ACKed", block); return PN7160_ERR; }
    /* Phase 2: the 16 data bytes; the card ACKs (0x0A) iff the write stuck. */
    uint8_t c2[1 + 16]; c2[0] = MFC_XCHG_HDR; memcpy(c2 + 1, in, 16);
    rn = 0;
    if (fn(ctx, c2, sizeof c2, rx, sizeof rx, &rn) < 0) return PN7160_ERR;
    if (!xchg_ack(rx, rn)) {
...
int mfc_value_cmd(apdu_fn fn, void *ctx, uint8_t op, uint8_t block, int32_t value)
{
    ...
    if (fn(ctx, c1, sizeof c1, rx, sizeof rx, &rn) < 0) return PN7160_ERR;
    if (!xchg_ack(rx, rn)) { LOGE("mfc: value op 0x%02x block %u command not ACKed", op, block); return PN7160_ERR; }
...
int mfc_transfer(apdu_fn fn, void *ctx, uint8_t block)
{
    ...
    uint8_t rx[16]; size_t rn = 0;
    if (fn(ctx, cmd, sizeof cmd, rx, sizeof rx, &rn) < 0) return PN7160_ERR;
    if (!xchg_ack(rx, rn)) { LOGE("mfc: transfer block %u failed", block); return PN7160_ERR; }
    return PN7160_OK;
}
...
int mfc_value_decode(const uint8_t in[16], int32_t *value)
{
    ...
    for (int i = 0; i < 4; i++) {
        if (in[i] != in[8 + i]) return PN7160_ERR;
        if (in[i] != (uint8_t)~in[4 + i]) return PN7160_ERR;
    }
    if (value)
