                                   ...
                                   const uint8_t key[16], desfire_lrp_session *s)
{
    ...
    if (desfire_apdu_raw(fn, ctx, 0x71, p1, sizeof p1, resp, sizeof resp, &rn, &status) != NCI_OK)
        return NCI_ERR;
    if (status != ST_AF || rn != 17) {
        LOGE("lrp: AuthLRPFirst part1 status 0x91%02x len %zu", status, rn);
        return NCI_ERR;
    }
    if (resp[0] != 0x01) { LOGE("lrp: card not in LRP mode (AuthMode 0x%02x)", resp[0]); return NCI_ERR; }
    ...
    if (desfire_apdu_raw(fn, ctx, 0xAF, p2, sizeof p2, resp, sizeof resp, &rn, &status) != NCI_OK)
        return NCI_ERR;
    ...
    if (memcmp(rmac, resp_mac, 16) != 0) { LOGE("lrp: response MAC mismatch"); return NCI_ERR; }
...
{
    if (!s || !s->active) { LOGE("lrp: no session"); return NCI_ERR; }
    ...
    if (data_len > 0 && tx_enc) {
        ...
        if (lrp_lricb(&s->ses, 1, c4, padded, pl, enc_data, 1) != 0) return NCI_ERR;
    ...
    while (status == ST_AF) {        /* native AF response chaining */
        if (rn >= sizeof resp) { LOGE("lrp: chained response overflow"); return NCI_ERR; }
        size_t more = 0; uint8_t st2 = 0;
        if (desfire_apdu_raw(fn, ctx, 0xAF, NULL, 0, resp + rn, sizeof resp - rn, &more, &st2) != NCI_OK)
            return NCI_ERR;
        rn += more; status = st2;
    }
    s->last_status = status;
    if (status != ST_OK) { LOGE("lrp: ins 0x%02x status 0x91%02x", ins, status); s->active = false; return NCI_ERR; }
    s->cmd_ctr++;
    ctr_lo = (uint8_t)(s->cmd_ctr & 0xFF); ctr_hi = (uint8_t)((s->cmd_ctr >> 8) & 0xFF);
    if (rn == 0) { if (out_len) *out_len = 0; return NCI_OK; }
    if (rn < 8) { LOGE("lrp: response missing MAC"); return NCI_ERR; }
    size_t erl = rn - 8;
    ...
    if (memcmp(rmact, resp_mac, 8) != 0) { LOGE("lrp: response MAC mismatch"); return NCI_ERR; }

    size_t produced;
    if (rx_enc && erl > 0) {
        if (erl % 16 != 0) { LOGE("lrp: resp not block-aligned"); return NCI_ERR; }
        uint8_t c4[4]; be32(c4, s->enc_ctr);
        uint8_t dec[512];
        if (lrp_lricb(&s->ses, 1, c4, resp, erl, dec, 0) != 0) return NCI_ERR;
...
int desfire_lrp_get_card_uid(apdu_fn fn, void *ctx, desfire_lrp_session *s, uint8_t uid[7])
{
    ...
    if (desfire_lrp_transact(fn, ctx, s, 0x51, NULL, 0, NULL, 0, false, true, buf, sizeof buf, &n) != NCI_OK)
        return NCI_ERR;
    if (n < 7) { LOGE("lrp: GetCardUID short (%zu)", n); return NCI_ERR; }
    memcpy(uid, buf, 7);
    return NCI_OK;
}
                          ...
                          uint32_t length, uint8_t *out, size_t out_cap, size_t *out_len)
{
    ...
    if (desfire_lrp_transact(fn, ctx, s, 0xAD, hdr, 7, NULL, 0, false, dec, buf, sizeof buf, &n) != NCI_OK)
        return NCI_ERR;
